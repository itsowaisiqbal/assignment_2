...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"src/Bottle.js":[function(require,module,exports) {
"use strict";
...
var Bottle = /*#__PURE__*/function (_Phaser$GameObjects$S) {
  _inherits(Bottle, _Phaser$GameObjects$S);

  var _super = _createSuper(Bottle);

  function Bottle(scene, x, y, sprite) {
    var _this;

    _classCallCheck(this, Bottle);

    _this = _super.call(this, scene, x, y, sprite); //when this class is accessed it access the current scene and adds the bottle to that scene 

    _this.scene = scene;
    scene.add.existing(_assertThisInitialized(_this)); //set the bottle interactive

    _this.setInteractive(); //set boolean values for logic use


    _this.flipped = false;
    _this.touch = false; //using phaser matter physics we turn our sprite into a gameobject

    scene.matter.add.gameObject(_assertThisInitialized(_this)); //when converted to a gameobject we are able to set its friction

    _this.setFriction(0.33, 0.01, 10);
  ...
  _createClass(Bottle, [{
    key: "bottle_stationary",
    value: function bottle_stationary() {
    ...
    key: "bottle_flip",
    value: function bottle_flip(mouse_pointer) {
      var velocity_y = (this.scene.startDragY - mouse_pointer) / 25;
      this.setVelocity(0, -velocity_y);
      this.setAngularVelocity(velocity_y / 360); //lock the velocity at 50 so our bottle stays under a limit
...
},{}],"src/MainScene.js":[function(require,module,exports) {
"use strict";
...
var score = 0; //set highscore to 0 if the user plays the game for the very first time or else store their previous value

var high_score = Number(localStorage.getItem("high_score")) || 0;
...
var MainScene = /*#__PURE__*/function (_Phaser$Scene) {
  ...
  _createClass(MainScene, [{
    ...
    value: function create() {
      ...
      this.height = this.game.config.height; //creating assets and adding it to our scene
    ...
    value: function update() {
      ...
      this.input.on("dragstart", function (mouse_pointer) {
        return _this.startDragY = mouse_pointer.y;
      });
      this.input.on("dragend", function (mouse_pointer) {
        _this.bottle.bottle_flip(mouse_pointer.y);

        _this.bottle.touch = true;
    ...
    key: "add_background",
    value: function add_background() {
      this.add.image(0, 0, "background").setOrigin(0, 0).setDisplaySize(this.width, this.height);
    }
  }, {
    key: "add_ground",
    value: function add_ground() {
      this.floor = this.add.image(0, this.height + 75, "ground").setOrigin(0, 1).setDisplaySize(this.width * 1.25, this.height / 1.75); //adds a rectangle box which acts as a collider for the bottle

      this.floorBody = this.matter.add.rectangle(this.width / 2, this.height + 300, this.floor.displayWidth, this.floor.displayHeight, {
        isStatic: true
      });
    }
  }, {
    key: "add_bottle",
    value: function add_bottle() {
      this.bottle = new _Bottle.Bottle(this, this.width / 2, this.height - 250, "bottle"); //converts the bottle gameobject into draggable so that our mouse pointer is able to access it. Check update function

      this.input.setDraggable(this.bottle);
    }
  }, {
    key: "game_score",
    value: function game_score() {
      //adds score to the scene
      this.scoreText = this.add.text(this.width / 2 - 25, this.height / 2 - 500, score, {
        fontFamily: 'Arial',
        fontSize: "100px",
        fontStyle: "bold"
      }); //adds high score to the scene

      this.scoreText = this.add.text(0, 0, "High Score: " + high_score, {
        fontFamily: 'Arial',
        fontSize: "50px",
        fontStyle: "bold"
      });
...
},{"./Bottle":"src/Bottle.js"}],"src/Main.js":[function(require,module,exports) {
"use strict";
...
},{"./MainScene":"src/MainScene.js"}],"../../../../../AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
